...
export interface CreateRecordParams
  extends
    ...
    DataCRUD.CreateOption<any> {}

const createRecordTool = tool(
  async ({ modelName, useApiName, data }: CreateRecordParams, request) => {
    const context = request.context as ShuttleAi.Cluster.Context

    const recordId = await context.dataModel
      .crud({
        modelName,
        useApiName,
        context,
  ...
  },
  {
    ...
    schema: z.object({
      ...
      data: z
        ...
        .catchall(z.any())
        .describe(
          'The record data to create. First fetch the field definitions of the model, then fill in the data according to those definitions. You do not need to provide _id, _createdAt, _updatedAt, _createdBy, or _updatedBy fields.',
        ),
    }),
    extras: {
      scope: 'write',
