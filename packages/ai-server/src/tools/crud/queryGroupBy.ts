...
export interface QueryGroupByParams
  extends
    Pick<DataCRUD.Server.Options, 'modelName' | 'useApiName'>,
    DataCRUD.QueryGroupByOption<any> {}

const queryGroupByTool = tool(
  async (
    { modelName, useApiName, ...groupByOptions }: QueryGroupByParams,
    request,
  ) => {
    const context = request.context as ShuttleAi.Cluster.Context

    const records = await context.dataModel
      .crud({
        modelName,
        useApiName,
        context,
  ...
  },
  {
    ...
    schema: z.object({
      ...
      aggFunction: z
        ...
        .describe('The aggregation function to apply to the grouped records.'),
      aggField: z
        .string()
        .describe('The field to apply the aggregation function to.'),
      condition: conditionPluginManager
        .getZod()
        .describe('The condition to filter records to find.')
        .optional(),
      // condition: z.object().catchall(z.any()).describe('Please call crud_get_condition_define to obtain the condition definition'),
      groupByFields: z
        .array(z.string())
        .describe('The fields to group by.')
        .optional(),
      orders: z
        .array(
          z.object({
            ...
            desc: z
              .boolean()
              .describe('Whether to order by descending order.')
              .optional(),
          }),
        )
        .describe('The orders to apply to the records.')
        .optional(),
      limit: z
        .number()
      ...
      offset: z
        .number()
        .describe('The offset to start returning records from.')
        .optional(),
    }),
    extras: {
      scope: 'read',
